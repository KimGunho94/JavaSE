...
[기본형식]
    ...
    WHERE 조건절
    GROUP BY 컬럼명
    HAVING 조건절 (GROUP 묶은 다음에 조건 줄 때)
    ORDER BY 컬럼명[ASC|DESC] -> 오름차순 혹은 내림차순
...
FROM employees
WHERE department_id = 90;  -- WHERE로 조건을 걸어 필터역활을 한다
...
FROM employees
WHERE manager_id IS NULL;   -- null 은 =(같음) 으로 검색이 되지 않는다.
...
SELECT INITCAP('higgins') FROM dual; -- 첫 글자를 대문자로 바꿈
/*
문자 조작 함수
*/
...
SELECT SUBSTR('HelloWorld', 1, 5) FROM dual; -- 1부터 5번째 글자 출력
...
/*
NVL2() 함수
    첫번째 표현식을 검사합니다. 첫번째 표현식이 null이 아니면 
    NVL2 함수는 두번째 표현식을 반환합니다.
